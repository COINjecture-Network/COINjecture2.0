...
\hypersetup{
  ...
  pdfauthor={Sarah Marin, Al Zalewski}
}
...
\begin{titlepage}
  ...
  \vspace{1.5cm}
  {\normalsize Whitepaper\\Version 2.6, May 30, 2026}

  \vspace{0.8cm}
  {\normalsize Authors: Sarah Marin (Founder), Al Zalewski}
...
formula:
\[
  \mathit{mint} = \left\lfloor \frac{w_{\mathrm{trunc}} \cdot S \cdot K}{W_{\mathrm{parent}}} \right\rfloor,
\]
where $w_{\mathrm{trunc}}$ is the truncated work score of the current block, $K$ is the emission
multiplier (set at $K=50$ and chosen to produce approximately 50~coins on the first block after genesis),
$S$ is the fixed-point scaling factor ($10^{12}$~atoms/coin), and $W_{\mathrm{parent}}$ is the parent
cumulative work. This mint issuance is separate from fork-choice weight (cumulative $w_{\mathrm{trunc}}$)
as reward does not affect security weight.
...
$w_{\mathrm{trunc}} / W_{\mathrm{parent}}$, even a successfully inflated work score would yield
