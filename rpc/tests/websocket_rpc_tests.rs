...
use coinject_rpc::websocket::{
    WebSocketRpc, RpcMessage, RpcEvent, RpcCommand, MiningWork, WorkQueue,
    ClientId,
};
use coinject_core::{ProblemType, Hash};
...
async fn test_rpc_message_serialization() {
    ...
    match deserialized {
        RpcMessage::Auth { client_id, signature } => {
...
async fn test_rpc_command_broadcast_block() {
    ...
    let result = cmd_tx.send(RpcCommand::BroadcastBlock {
        height: 100,
        hash,
    });
    assert!(result.is_ok());
...
async fn test_rpc_event_types() {
    ...
    use coinject_core::{TransferTransaction, Ed25519Signature};
