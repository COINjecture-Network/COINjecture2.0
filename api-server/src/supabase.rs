...
impl SupabaseClient {
    ...
    fn write_key(&self) -> Result<&str, SupabaseError> {
        self.service_key
            .as_deref()
    ...
    /// Generic PostgREST INSERT for multiple rows.
    pub async fn insert_rows(&self, table: &str, body: Value) -> Result<Value, SupabaseError> {
        let key = self.write_key()?;
    ...
    pub async fn delete_rows(&self, path: &str) -> Result<(), SupabaseError> {
        let key = self.write_key()?;
        let resp = self
            .http
            .delete(format!("{}/rest/v1/{path}", self.url))
            .header("Authorization", format!("Bearer {key}"))
            .header("apikey", &self.anon_key)
            .send()
            .await
    ...
    pub async fn get_dataset_catalog(&self) -> Result<Value, SupabaseError> {
        let key = self.service_key.as_deref().unwrap_or(&self.anon_key);
        self.postgrest_get_with_key(
            "dataset_catalog?select=*,latest_snapshot:dataset_snapshots!latest_snapshot_id(*)&order=created_at.desc",
            key,
        )
        .await
    }
    ...
    pub async fn get_solution_sets(
        ...
        let allowed_order = if sort_order.eq_ignore_ascii_case("asc") {
            "asc"
        } else {
            "desc"
        };
        ...
        let mut path = format!(
            "solution_sets?select=*&order={allowed_sort}.{allowed_order}.nullslast&limit={limit}"
        );

        if let Some(problem_type) = problem_type {
            path.push_str(&format!("&problem_type=eq.{problem_type}"));
        }
