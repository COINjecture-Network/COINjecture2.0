                            ...
                            match chain.store_block(&block).await {
                                Ok(is_new_best) => {
                                    ...
                                    if is_new_best || extends_chain {
                                        ...
                                        match Self::apply_block_transactions(
                                            ...
                                            Ok(applied_txs) => {
                                                info!(
                                                    ...
                                                    tau = tau,
                                                    "buffered block applied to chain"
                                                );
                                        ...
                                        warn!(
                                            block_height = next_height,
                                            best_height = current_best,
                                            "buffered block does not extend best chain, skipping"
                                        );
                                        // Continue loop to check for next sequential block
                                    }
                                }
                ...
                None => {
                    ...
                    if let Some(&max_buffered_height) = buffer.keys().max() {
                        if max_buffered_height > next_height {
                            ...
                            warn!(
                                block_height = next_height,
                                max_buffered = max_buffered_height,
                                "missing block, requesting from peers"
                            );
    ...
    pub(crate) fn unwind_block_transactions(
        ...
        if current_balance >= reward {
            ...
            warn!(
                ...
                reward = reward,
                "miner balance below reward, setting to zero"
            );
            state
                .set_balance(&miner, 0)
                .map_err(|e| format!("Failed to set miner balance: {}", e))?;
            ...
            coinject_core::Transaction::Channel(channel_tx) => {
                match &channel_tx.channel_type {
                    ...
                    ChannelType::UnilateralClose { .. } => {
                        ...
                        warn!(
                            "cannot perfectly reverse unilateral close, state may be inconsistent"
                        );
                        Ok(())
                    }
                }
