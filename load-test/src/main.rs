...
#[command(
    name = "load-test",
    about = "COINjecture load and stress testing harness"
)]
#[command(version)]
struct Cli {
    #[command(subcommand)]
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::from_default_env().add_directive("load_test=info".parse().unwrap()),
        )
        .init();
    ...
    if cli.json {
        ...
        if let Some(path) = cli.output {
            let json =
                serde_json::to_string_pretty(&results).expect("results serialization failed");
            std::fs::write(&path, json).expect("failed to write output file");
            tracing::info!("results written to {}", path);
        }
    }
