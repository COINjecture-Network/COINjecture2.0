...
impl StakingMetrics {
    ...
    pub fn bootstrap(blocks_per_year: u64) -> Self {
        ...
        // D_n = e^(-η * n) for n = 1..8
        let avg_scale: f64 = (1..=8).map(|n| (-ETA * n as f64).exp()).sum::<f64>() / 8.0;
    ...
    pub fn update_from_network(
        ...
        if total > 0 {
            let weighted_avg: f64 = pool_totals
                .iter()
...
mod tests {
    ...
    fn test_delta_critical_mathematical() {
        ...
        assert!(
            ...
            "Delta critical should be mathematical: {} vs {}",
            delta,
            expected
        );
    }
    ...
    fn test_base_yields_dimensionless() {
        ...
        for pool_type in PoolType::all() {
            ...
            assert!(
                yield_rate > 0.0 && yield_rate <= 1.0,
                "Yield for {:?} should be ratio: {}",
                pool_type,
                yield_rate
            );
        }
    ...
    fn test_viviani_multiplier_no_hardcoded_max() {
        ...
        assert!(
            portfolio.multiplier < 1.0 + delta_critical() + 0.01,
            "Multiplier should be bounded by math: {}",
    ...
    fn test_target_eta_calculated() {
        ...
        let expected: f64 = (1..=8).map(|n| (-ETA * n as f64).exp()).sum::<f64>() / 8.0;

        assert!(
            (target - expected).abs() < 0.001,
            "Target η should be calculated: {} vs {}",
            target,
            expected
        );
    }
    ...
    fn test_hhi_calculation() {
        ...
        assert!(
            (lambda_concentrated - 0.0).abs() < 0.01,
            "Concentrated λ should be ~0: {}",
            lambda_concentrated
        );
        ...
        assert!(
            (lambda_diversified - 0.875).abs() < 0.01,
            "Diversified λ should be ~0.875: {}",
            lambda_diversified
        );
    }
    ...
    fn test_network_derived_target() {
        ...
        assert!(
            ...
            "Network target should be valid ratio: {}",
            target
        );
    }
}
