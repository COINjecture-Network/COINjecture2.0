...
use coinject_core::{Block, BlockHeader, Hash, Address, CoinbaseTransaction, SolutionReveal, Commitment};
use coinject_network::reputation::{ReputationManager, FaultType};
...
async fn test_cpp_block_propagation() {
    ...
    MessageCodec::send_new_block(&mut client_stream, &new_block).await.unwrap();
...
async fn test_cpp_sync_get_blocks_request() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        MessageCodec::send_blocks(&mut stream, &blocks_msg).await.unwrap();
    });
    ...
    MessageCodec::send_get_blocks(&mut client_stream, &get_blocks).await.unwrap();
...
async fn test_cpp_sync_large_block_batch() {
    ...
    for i in 0..blocks_msg.blocks.len() - 1 {
        ...
        assert_eq!(current_hash, next_prev_hash, "Chain integrity broken at height {}", i);
    }
