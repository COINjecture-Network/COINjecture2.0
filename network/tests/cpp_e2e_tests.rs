...
use coinject_network::cpp::{
    CppNetwork, NetworkEvent, NetworkCommand, CppConfig, NodeType as CppNodeType,
};
use coinject_core::{Hash, Block, BlockHeader, Address, CoinbaseTransaction, SolutionReveal, Commitment};
...
async fn test_network_broadcast_and_receive_block() {
    ...
    cmd_tx.send(NetworkCommand::BroadcastBlock { block: block.clone() }).unwrap();
...
async fn test_network_chain_state_update() {
    ...
    cmd_tx.send(NetworkCommand::UpdateChainState {
        best_height: 100,
        best_hash: new_hash,
    }).unwrap();
...
async fn test_network_request_blocks() {
    ...
    // Request blocks
    cmd_tx.send(NetworkCommand::RequestBlocks {
...
async fn test_network_command_handling() {
    ...
    let block = create_test_block(1, genesis);
    cmd_tx.send(NetworkCommand::BroadcastBlock { block }).unwrap();
    
    use coinject_core::{TransferTransaction, Ed25519Signature, PublicKey};
    ...
    cmd_tx.send(NetworkCommand::BroadcastTransaction { transaction: tx }).unwrap();
    ...
    cmd_tx.send(NetworkCommand::RequestHeaders {
        peer_id: [2u8; 32],
        from_height: 0,
        to_height: 100,
        request_id: 1,
    ...
    cmd_tx.send(NetworkCommand::DisconnectPeer {
        peer_id: [2u8; 32],
        reason: "test".to_string(),
    }).unwrap();
